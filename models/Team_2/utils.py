...
#### Augmentations
def shift_image(img, shift_pnt, borderMode=cv2.BORDER_CONSTANT, borderValue=(255,255,255)):
    M = np.float32([[1, 0, shift_pnt[0]], [0, 1, shift_pnt[1]]])
    res = cv2.warpAffine(img, M, (img.shape[1], img.shape[0]), borderMode=borderMode, borderValue=borderValue) #BORDER_WRAP  cv2.BORDER_REFLECT_101
    return res


def rotate_image(image, angle, scale, rot_pnt, borderMode=cv2.BORDER_CONSTANT, borderValue=(255,255,255)):
    rot_mat = cv2.getRotationMatrix2D(rot_pnt, angle, scale)
    result = cv2.warpAffine(image, rot_mat, (image.shape[1], image.shape[0]), flags=cv2.INTER_LINEAR, borderMode=borderMode, borderValue=borderValue) #INTER_NEAREST BORDER_REFLECT_101
    return result
...
def preprocess_inputs_float(x):
    x = np.asarray(x, dtype='float32')
    x /= 0.5
    x -= 1
    return x

def preprocess_inputs_rgb(x, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
...
def dice(im1, im2, empty_score=1.0):
    ...
    if im1.shape != im2.shape:
        raise ValueError("Shape mismatch: im1 and im2 must have the same shape.", im1.shape, im2.shape)
